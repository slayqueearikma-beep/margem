...
import { externalHref, truncate } from "@/lib/format";
  ...
  try {
    ...
    if (!sellerOutcome.ok) {
      return buildPageMetadata({
        title: t("sellerNotFound.title"),
        description: t("sellerNotFound.description"),
        path: `/sellers/${id}`,
        locale,
      });
    }
...
export default async function SellerDetailPage({ params }: SellerDetailProps) {
  ...
  const tDetail = await getTranslations("sellerDetail");
  const tFormat = await getTranslations("format");
  const { formatRating, verificationLabel } = createFormatters(locale, {
    ...
    <div className="space-y-10">
      ...
      <section className="overflow-hidden rounded-3xl border border-[var(--border)] bg-white">
        ...
        <div className="space-y-4 p-6 sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="text-end">
              ...
              <p className="text-sm text-[var(--muted)]">
                {tDetail("reviewCount", { count: seller.review_count })}
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-3 text-sm text-[var(--muted)] sm:grid-cols-2">
            <p>{seller.address}</p>
            <p>{seller.city}</p>
            {seller.phone ? <p>{tCommon("phone", { phone: seller.phone })}</p> : null}
            {website ? (
              <p>
                {tCommon("website")}{" "}
                <a
      ...
      {seller.products.length > 0 ? (
        <section className="space-y-4">
          <h2 className="text-xl font-semibold">{tCommon("products")}</h2>
      ...
      {seller.services.length > 0 ? (
        <section className="space-y-4">
          <h2 className="text-xl font-semibold">{tCommon("services")}</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      {reviews.length > 0 ? (
        <section className="space-y-4">
          <h2 className="text-xl font-semibold">{tCommon("reviews")}</h2>
            ...
            {reviews.slice(0, 6).map((review) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <p className="text-sm text-[var(--muted)]">
                    {review.overall_rating.toLocaleString(
