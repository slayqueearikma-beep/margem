"""Simplify marketplace: customer/provider roles, pricing_type, fundamental categories.
...
def upgrade() -> None:
    # Rename marketplace account roles (admin/support unchanged).
    ...
    # Migrate legacy negotiable / empty price rows to "offer".
    op.execute(
        """
        UPDATE products
    ...
    # Infer delivery flags from legacy delivery_options JSON text.
    op.execute(
        """
        UPDATE products
    ...
    # Normalize city to Casablanca for the single-city launch.
    op.execute("UPDATE seller_profiles SET city = 'Casablanca' WHERE city IS NULL OR city = ''")

    # Replace category taxonomy with fundamental retail verticals.
    ...
    # Remap deprecated product category slugs.
    op.execute(
        """
        UPDATE products SET category_slug = 'home'
        WHERE category_slug = 'services' OR category_slug = 'home-garden'
        """
    )
    op.execute(
        """
